...
# - image itself is stored in $$exifTool{$tag}
sub GetImageType($$$)
{
    my ($exifTool, $val, $tag) = @_;
    my ($w, $h) = @$exifTool{"${tag}Width","${tag}Height"};
    my $type = 'DAT';
    ...
    if ($val =~ /^\x89PNG\r\n\x1a\n/) {
        ...
        $exifTool->Warn("Unrecognized FLIR $tag data format");
    } elsif (GetByteOrder() eq 'II') {
        require Image::ExifTool::Sony;
        $val = Image::ExifTool::Sony::MakeTiffHeader($w,$h,1,16) . $val;
        $type = 'TIFF';
    } else {
        $exifTool->Warn("Don't yet support big-endian TIFF $tag");
    }
    # save image data
    $$exifTool{$tag} = $val;
    return $type;
